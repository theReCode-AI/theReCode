...
def test_build_therecode_workflow_compiles_graph() -> None:
    ...
    assert "gate_risk_approval" in node_names
    assert "fix_planner_agent" in node_names
    assert "finalize_run" not in node_names
    assert "code_fix_agent" not in node_names


def test_build_post_risk_workflow_includes_fix_and_finalize() -> None:
    from app.google_adk.workflow_builder import build_post_risk_approval_workflow

    workflow = build_post_risk_approval_workflow(model="gemini-2.5-flash")
    node_names = {node.name for node in workflow.graph.nodes}
    assert "code_fix_agent" in node_names
    assert "peer_review_agent" in node_names
    assert "finalize_run" in node_names
    assert "gate_risk_approval" not in node_names
